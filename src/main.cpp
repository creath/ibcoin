...
uint256 hashGenesisBlock("0x00000d9bfa4ebc8c871783af3a7018dc49c648a56a65422d993830b60dea4c77");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // IBCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "IBCoin Signed Message:\n";
...
{
    // IBCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 0.8 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	static const int64	BlocksTargetSpacing			= 0.8 * 60;
	unsigned int		TimeDaySeconds				= 60 * 60 * 24;
	int64				PastSecondsMin				= TimeDaySeconds * 0.8 * 0.1;
	int64				PastSecondsMax				= TimeDaySeconds * 0.8 * 2.8;
...
{
    // IBCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "ibc genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1485102656;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1942784;
        ...
        assert(block.hashMerkleRoot == uint256("0x4d01f7af4ee1be681465d00bcbde53c33e65b30042db9a66482e6c791a6b5501"));
...
unsigned char pchMessageStart[4] = { 0xfe, 0xd3, 0xb3, 0xde }; // IBCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IBCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IBCoinMiner : ProcessBlock, block not accepted");
    }
